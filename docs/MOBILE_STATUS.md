...
- **更替項目備援記錄完成（階段 1.5-B，B-4c 已完成）**：更替頁對 `isWaitingForTask` 為 `true`（含查詢狀態未知時一律不算）的項目，在項目列下方加一個小型文字連結「記錄完成」（非顯眼主按鈕，沿用既有 inline style）；點擊後在同一列展開二次確認區塊（不用瀏覽器原生 `confirm()`、不加新套件），文案明確告知「記錄後手機無法取消，若要反悔需到桌面復原」，取消不做任何寫入。確認後 `useMaintenanceItems.recordFallbackCompletion`（`src/hooks/useMaintenanceItems.ts`）依序執行：(a) 用 `item.id` 重新對 `maintenance_items`（`deleted_at is null`）做一次全新 `select`，不信任畫面上的舊資料，查無則中止並提示；(b) `today = getDailyDateKey()`、`newNextDueAt = calcNextDueAt(today, item.cycle_value, item.cycle_unit)`（沿用 B-2 那份，未另寫），兩者皆須通過 `PLAIN_DATE_RE`（`/^\d{4}-\d{2}-\d{2}$/`，從 `useTasks.ts` 匯出重用、未重寫第二份）驗證才繼續；(c) UPDATE `maintenance_items` 的 `last_handled_at` / `next_due_at` / `updated_at`，這是整段流程的第一個寫入，失敗即中止、之前沒有任何東西需要回捲；(d) INSERT 一筆 `maintenance_events`（`task_id: null`，因為整段流程完全不讀寫 `tasks`；`generated_next_task_id: null`；不含任何 `generation_key` 欄位），此步失敗不回捲 (c)，只提示「已推進週期，但紀錄寫入失敗」。**全程對 `tasks` 零讀寫**。成功（或 (c) 成功但 (d) 失敗）後一律呼叫既有的 `reload()`（即 `load()`）重新整份讀取，讓新的 `next_due_at` 與「等待」標記立即反映，因此送出當下會短暫回到整頁 loading 畫面（沿用既有 `load()` 行為，未另寫局部更新）。完全成功時的提示文字為「已推進週期，下一輪任務會在桌面同步後產生」，走新增的 `useMaintenanceItems` 內 `actionNotice`（綠色、3.5 秒自動清除，命名與樣式沿用 `useTasks` 既有的 `actionError`/`actionNotice` 慣例，但這是獨立的一組 state，不是同一個 hook 實例）；真正失敗訊息維持紅色 `actionError`。（提示計時與並發寫入的修正見下方 B-4c-1。）
- **修正備援入口並發寫入與提示時序（階段 1.5-B，B-4c-1 已完成）**：純邏輯修正，未新增任何寫入路徑。
  - **防止並發重複寫入**：`confirmingId`／`recordingId` 原本是頁面層級各自獨立的單一值，在某一項目寫入中時點擊另一項目的「記錄完成」會把 `confirmingId` 換走，讓正在寫入中的項目重新顯示連結、可能被二次觸發並行寫入。改為新增 `requestConfirm`（`src/pages/Maintenance.tsx`）：只要 `recordingId` 非空就直接拒絕開新的確認框；同時把 `linkDisabled={recordingId !== null}` 傳給每一列 `ItemRow`，讓所有項目的「記錄完成」連結在任何一項寫入中時一律停用（灰字、`disabled`、不可點擊），寫入結束後才恢復，而非隱藏。
  - **成功提示不再被 reload 吃掉計時**：`useMaintenanceItems.load()` 改為回傳 `boolean`（`true`＝主要的 `maintenance_items` 讀取成功）。`recordFallbackCompletion` 改成先 `await load()` 完成、確定畫面已離開 loading 狀態，才呼叫 `setActionNotice`／`setActionError`，避免原本「先設訊息、reload 使 `loading` 變 true、`Maintenance()` 提早 `return <LoadingScreen />` 導致 toast 從未真正顯示、卻已開始倒數 3.5 秒」的問題（慢連線下可能整個看不到提示）。
  - **reload 失敗會被看到**：若寫入本身成功但緊接著的 `load()` 失敗（`reloadOk === false`），改用既有的紅色 `actionError` 顯示「已推進週期，但畫面重新整理失敗，可能未顯示最新狀態，請下拉重新整理」，取代原本會被綠色成功提示蓋掉的情況；沒有新增第三種 toast 樣式，仍只有 `error`（紅）/`notice`（綠）兩種。
- **修正備援入口鎖定可能卡死（Phase 1.5-B 收尾）**：`Maintenance.tsx` 的 `handleConfirmRecord` 原本沒有 `try/finally`，若 `recordFallbackCompletion` 拋出未捕捉例外（例如離線時底層 fetch 直接 reject，或某筆項目資料異常導致 `calcNextDueAt` 內 `Invalid Date.toISOString()` 拋出 `RangeError`），`recordingId` 永遠不會被清成 `null`，而 B-4c-1 的鎖定設計是全域的（`recordingId` 非空時所有項目的「記錄完成」連結都停用），後果會是整頁功能永久卡死、只能重新整理頁面解除。已改成 `try { await recordFallbackCompletion(itemId) } catch { ... } finally { setRecordingId(null); setConfirmingId(null) }`，確保無論成功、正常失敗或例外都一定會解鎖；`catch` 區塊另外設定本頁新增的 `fallbackError` 狀態（3.5 秒自動清除，走既有紅色 `error` 樣式，不新增第三種 toast），顯示「暫時無法完成，請稍後再試」，不再靜默吞掉例外。只動這一個函式與其對應的最小提示狀態，未調整 B-4c-1 的鎖定設計本身（`confirmingId`／`recordingId` 語意不變）。
...
## Phase 1.5-B 階段還原點
> 若某階段上線後需要回退，可用下列 commit hash 對照回退目標；commit 內容
> 請用 `git show <hash> --stat` 或 `git log <hash> -1` 查閱，本清單只記錄
> 對照表，不重複描述每個 commit 做了什麼（詳見上方「目前已實作的功能」對應
> 段落）。

| 階段 | 內容 | commit hash |
|---|---|---|
| B-1 | 唯讀列表 | `b886214`（前一版：`ec823a3`） |
| B-2 | 新增項目 | `799e6a4` |
| B-4a | 完成更替任務 | `3ca67ee` |
| B-4b | 標記與刪除防護 | `3029f64` |
| B-4b-1 | 修正等待標記誤報 | `7b681c0` |
| B-4c | 備援記錄完成 | `5f413c3` |
| B-4c-1 | 修正並發與提示時序 | `df2d2fb` |

桌面對應版本：v1.4.20260806（含 post-pull 補產生）。

## 更替功能驗收清單
> 供日後改版做回歸測試重跑；以手動操作描述「去哪一頁、做什麼、應該看到
> 什麼」，不寫變數名或函式名。

1. 打開「更替」分頁，確認畫面列出目前啟用中的項目，每筆都顯示下次到期日與剩餘天數；已逾期的項目要有紅色的視覺區隔（框線或文字顏色），與其他項目明顯不同。
2. 到「全部任務」或「今日焦點」頁，確認由更替項目產生的任務標題旁邊有一個綠色葉子小圖示；一般手動建立的任務標題旁邊沒有這個圖示。
3. 在「全部任務」或「今日焦點」頁長按一筆更替任務，確認畫面跳出「無法在手機刪除、請到桌面刪除」之類的提示，且該任務仍留在清單中、沒有消失；再長按一筆一般任務，確認可以正常刪除（出現「已刪除」提示，且該任務從清單消失）。
4. 到「更替」分頁按「＋ 新增」，在週期欄位輸入小數（例如 1.5）或非數字，確認畫面顯示錯誤提示且無法送出；改輸入正整數（例如 7）後可以正常送出成功。
5. 找一筆已經有對應任務的更替項目，把該任務標記完成，回到「更替」分頁確認這筆項目的下次到期日已經往後推進，且畫面出現的提示是綠色（不是紅色）。
6. 找一筆「尚未有對應任務」的更替項目（或等桌面同步跑一輪，讓某筆項目變成這個狀態），確認畫面上出現等待桌面產生任務的提示文字，並且下方多了一個「記錄完成」的入口。
7. 按下「記錄完成」，確認畫面先跳出二次確認的文字說明、不會立刻執行；按「取消」後確認該項目的到期日與狀態都沒有改變。
8. 對某一筆項目按「記錄完成」並確認送出後，趁畫面還在處理時查看清單，確認「所有」項目的「記錄完成」入口都變成無法點擊的停用狀態，不是只有正在處理的那一筆。
9. 在桌面版對同一使用者帳號同步一次，確認該更替項目底下只多出一筆未完成任務，而不是兩筆或以上（驗證手機端沒有重複產生、桌面端補產生邏輯正常）。
